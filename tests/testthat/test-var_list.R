...
test_that("var_list() works", {
  ...
  vcr::use_cassette("var_list", {
    vars <- CLDassist::var_list(topic = "addiction",
                                n_final = 2,
                                n_variables = 2)
  })
...
test_that("Invalid topic parameter causes error", {
  testthat::expect_error(CLDassist::var_list(topic = 123,
                                             n_final = 5,
                                             n_variables = 10,
                                             LLM_model = "gpt-4",
                                             max_tokens = 1000),
...
test_that("Invalid n_variables parameter causes error", {
  testthat::expect_error(CLDassist::var_list(topic = "Climate Change",
                                             n_final = 5,
                                             n_variables = -1,
                                             LLM_model = "gpt-4",
                                             max_tokens = 1000),
