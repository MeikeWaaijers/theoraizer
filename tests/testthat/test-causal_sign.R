...
test_that("causal_sign() works for a both input df", {
  ...
  vcr::use_cassette("causal_sign", {
    rel_sign <- CLDassist::causal_sign(topic = "addiction",
                                       prob_df = CLDassist::rel$relation_df[1,])
    dir_sign <- CLDassist::causal_sign(topic = "addiction",
                                       prob_df = CLDassist::dir$direction_df[1,])
...
test_that("Invalid topic parameter causes error", {
  ...
  testthat::expect_error(CLDassist::causal_sign(topic = 123,
                                                prob_df = prob_df,
                                                causal_threshold = 50,
                                                LLM_model = "gpt-4",
                                                max_tokens = 1000),
...
test_that("Invalid prob_df parameter causes error", {
  testthat::expect_error(CLDassist::causal_sign(topic = "Climate Change",
